...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant = 'primary', size = 'md', children, ...props }, ref) => {
    ...
    const glowEffect =
      variant !== 'outline' ? 'hover:scale-105 shadow-lg hover:shadow-xl' : 'hover:scale-105';

    return (
      <button
        className={cn(baseStyles, variantStyles[variant], sizeStyles[size], glowEffect, className)}
        ref={ref}
        {...props}
      >
        {children}
